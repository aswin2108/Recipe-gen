...
export class RecipeComponent implements OnInit{
  constructor(private mealsService:MealService, private sanitize:DomSanitizer, private spinner: NgxSpinnerService){}
  ...
  fetchData() {
    ...
    this.mealsService.getRecipe().subscribe({
      next(data: {[key:string]:any}) {
        console.log(typeof(data['meals'][0].idMeal));
        console.log(data);
        ...
        const cleanUpData = () => {
          ...
          for (const key in data['meals'][0]) {
            if (key.includes('strIngredient')) {
              if(data['meals'][0][key as keyof RawMealData]!=='' && data['meals'][0][key as keyof RawMealData]!==null)
              ...
              if(insrtuctions.length===1)insrtuctions=data['meals'][0][key as keyof RawMealData].split('\r\n' || '.');
            }
  ...
  convertUrl(url:string):void{
    ...
    this.newMeal.strYoutube=this.sanitize.bypassSecurityTrustResourceUrl(sanitizedUrl);
    console.log(this.newMeal.strYoutube);
